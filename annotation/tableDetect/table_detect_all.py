...
def recognize_structure(img):
    ...
    for c in contours:
        ...
        if (
            w < 0.9 * img_width
            and h < 0.9 * img_height
            and (w > max(10, img_width / 30) and h > max(10, img_height / 30))
        ):
    ...
    cv2.imwrite(str(BASE_DIR) + "\\media\\masked_table.jpg", img_vh)
    # cv2.imshow(img_vh)
